...
class LoginSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        captcha_key = self.initial_data.get("captchaKey", None)
        flag = dispatch.get_system_config_values("base.captcha_state")
        if flag:
            if not captcha_key or not captcha:
                raise CustomValidationError("验证码不能为空")
            self.image_code = CaptchaStore.objects.filter(id=captcha_key).first()
