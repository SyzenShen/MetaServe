...
MetaServe项目面向科研机构或企业级生物信息中心，用于补充共享存储 / NAS 以提供治理与交付层，实现实验数据的可视化与追踪。且预留了基于机器学习的智能分析接口，可拓展科研数据的智能标注与可视化。

MetaServe is aimed at research institutions or enterprise-level bioinformatics centers, and is designed to supplement shared storage / NAS with a governance and delivery layer to achieve the visualization and tracking of experimental data. A machine learning–based intelligent analytics interface is reserved for extensible data annotation and visualization in scientific research.

# MetaServe

**MetaServe** 是面向现代生物实验室的数据管理系统，旨在应对高通量测序产生的海量数据及其派生文件带来的组织与共享挑战。系统基于轻量的 Django + Vue 架构，整合文件索引、元数据检索、权限管理与高可靠传输，提供统一入口以追踪样本与文件的全生命周期，并以结构化方式管理元数据且可从NCBI下载文件。在此基础上，系统引入智能分析占位接口（Intelligent Analytics Interface），为自动标签、质控检测、摘要生成、向量检索及 .h5ad 预处理等 AI/ML 能力预留统一接入点，支持未来算法模块的灵活扩展与替换。相比传统脚本流与共享盘，MetaServe 通过统一的元数据层、智能化分析入口与精细化访问控制机制，可提升生物数据的可检索性、自动化程度与治理能力。系统与 Cellxgene 集成，支持 `.h5ad` 文件的即时可视化，实现从原始数据到下游分析的衔接。在实际部署中，该系统可降低跨团队协作的复杂度，并通过细粒度权限控制与结构化元数据管理提供数据的可追溯性支持。
...
Building upon this foundation, MetaServe introduces an Intelligent Analytics Interface (ml_interface) — a unified gateway reserved for future AI/ML modules such as auto-tagging, quality control, summary generation, vector embedding, and .h5ad preprocessing. This design enables integration of advanced analytics capabilities and ensures that new algorithms can be flexibly plugged in without disrupting existing workflows.

Compared with traditional script-based pipelines and shared drives, MetaServe can improve the searchability, automation, and governance of biological data through its unified metadata layer, intelligent analytics entry point, and fine-grained access control. Integration with **Cellxgene** enables instant visualization of `.h5ad` files, providing a bridge from raw data to downstream analysis. In practical deployments, the system can reduce cross-team collaboration complexity and provides data traceability support through fine-grained access control and structured metadata management.
...
| Facets 多维检索 Faceted Search | 支持物种 × 实验 × 标签 × 权限等组合筛选。<br>Supports combined filtering by Species × Experiment × Tags × Permissions. |
| 访问控制与审计 Access Control & Audit | 公共 / 内部 / 私有三级策略，继承到用户、组、项目或目录级，有操作日志记录。<br>Three-level policy (Public/Internal/Private), inherited by user, group, project, or directory; operation logging is available. |
...
- **Zero-copy Pipeline Access**: 新增 `/api/files/manifest/` 接口，支持分析流程通过文件路径清单直接读取数据（无需下载）。
  <br>Added `/api/files/manifest/` endpoint, supporting analysis pipelines reading data directly via file path lists (no download required).
- **Cellxgene 优化 (Cellxgene Optimization)**: 采用 Symlink 软链接发布 `.h5ad` 文件，避免数据冗余。
  <br>Uses Symlink for publishing `.h5ad` files to avoid data redundancy.
- **工程化部署 (Engineering Deployment)**: 提供 Docker Compose 生产环境配置。
  <br>Provides Docker Compose production environment configuration.
...
性能测试相关脚本与数据见 `performance_test_project/`。
<br>Performance test scripts and data are available in `performance_test_project/`.
...
| --- | --- |
| 数据加密 Data Encryption | 传输层支持 HTTPS/TLS；断点续传使用带签名的临时 URL。<br>HTTPS/TLS supported for transport; signed temporary URLs for resumable transfers. |
| 访问审计 Access Audit | 登录、文件 CRUD、权限变更等操作有日志记录；支持基于用户、时间窗口的查询。<br>Logging for login, file CRUD, permission changes; supports querying by user, time window. |
| 保留与删除 Retention & Deletion | 提供“软删除”功能。<br>Provides "soft delete" functionality. |
| 安全记录 Security Records | 支持下载记录导出。<br>Supports download record export. |
